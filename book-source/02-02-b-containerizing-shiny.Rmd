...
Alpine Linux is a minimal distribution independent of Debian and other
...
comes down to minimalism vs. convenience. Ubuntu and its derivatives tend to be
much larger in size, but build times can be considerable faster due to very
...
parent image tend to share lost of their layers anyways, so the images can pack 
much tighter on your hard drive then you might think based on their size without 
subtracting overlapping layers. Also,the size advantage of the Alpine 
...
Let's see some of the most popular base R images based on `docker images`:
...
The `rocker/r2u` is based Ubuntu as well, and it brings Ubuntu binaries for
...
Finally, the `rhub/r-minimal` image is based on Alpine and is the tiniest 
...
that brings versioned base R to many Linux distributions, e.g. CentOS,
...
dependency becomes `libcurl4`. The run time dependency tends to be much smaller
and it has fewer dependencies. It will have no conflict with other run time
libraries because of the lack of headers included.

### Manually Installation

FIXME: Kalvin to review the Python specific parts

The [Python Wheels](https://pythonwheels.com/)
...
Linux distributions bor the obvious complexity involved in that.
The [Posit Public Package Manager](P3M, <https://p3m.dev/>) provides pre-built binary 
...
The R Universe project only provide binaries for R packages on Ubuntu 
...
lists system requirements for R packages: 
...
Linux distributions. The `GET` request will result in a JSON response listing 
...
you might end up with an outdated version of the packages.

### Automated Dependency Resolution with r2u

If you are using R on Ubuntu, the [r2u](https://eddelbuettel.github.io/r2u/)
...
For the binary packages are base on P3M where available or built natively. 
Selected BioConductor packages are also built natively on the project servers. 
The server hosting the `.deb` files is set up as a proper `spt` repository
...
image has no X11 grahic device support that is used to display base R plots, like histograms.
...
increase in the Ubuntu based image's size is similar, but it less noticeable
in relative terms.
...
Not relevant for Linux containers, but `pak` even can handle locked package DLLs 
...
You can pin version using Git references, e.g. `tidyverse/ggplot2@v1.0.0`,
...
repository is a repository for recently archived CRAN. 
Then `COPY` this file into the `/etc/R` folder on the Rocker images:

```
COPY Rprofile.site /etc/R
```

As you develop your Shiny app, you have to occasionally update the `Dockerfile`
to add in any new dependencies.
...
not necessarily structured as a package. The only think you need for
...
Using `DESCRIPTION` file lets you record the dependencies
outside of your `Dockerfile`. As you develop your Shiny app, you have to update 
the `DESCRIPTION`. When the file changes, Docker fill invalidate the cache and
...
With the "custom" model lets you specify filters for modifying the implicit
...
Read the _Using renv with Docker_ vignette of `renv` for more useful tips.
...
If you are developing your Shiny app as an R package, then the the 
...
versioning requirements without having to manage a `DESCRIPTION` file.
Because why would you need a `DESCRIPTION` file when you have no R package? 
...
It writes these dependencies `dependencies.json` file, including  the information 
contained in the comments when using the `deps::create()` function.
...
comments. The other function is `install()` which looks for the `dependencies.json`
...
parse the JSON and install any of these libraries. Finally, the 
...
FIXME: KALVIN to add content here

### Other options for python

??? Conda, venv ???
Docker's purpose is isolation, so multiple venv's do not make sense in containers according to some posts. Maybe explain that here?

FIXME: KALVIN to add content here

FIXME: mention shinylive installs

FIXME: how to reuse `.cache` by mounting it: <https://testdriven.io/blog/docker-best-practices/#cache-python-packages-to-the-docker-host>
...
but used the `DESCRIPTION` file to define its dependencies that will be picked
...
this Here is the one with Golem:
...
The Python version of the app follows the similar principles. You can find it
in the `py-shiny` folder.
The parent image is `python:3.9`, and dependency management is done via `pip` and 
the `requirements.txt` file. Edit this file as the starting point for your single
or multiple file Python for Shiny apps The `CMD` instruction calls `uvicorn`
to host the app:
...
install with the image operating systems' package manager. The `CMD` instruction
uses `rmarkdown::run()` to render and run the document every time a user connections
to it:
...
The prerendered option will with `runtime: shinyrmd` require a rendering step 
...
Markdown. First we install `quarto`. There are many ways of installing it,
this is one option. You need `curl` to download the Quarto installer,
we use here `gdebi` install Quarto from the downloaded `.deb` package.
...
FIXME: find a way where this setup should be. Probably an independent 
h10y/examples repo where we can have the setup scripts etc for this one and 
docker compose.
...
different apps inside the nested folders.
...
The `Dockerfile` sits next to the `apps` folder, and here are the contents.
...
```{r part2-docker-shinyserver, eval=TRUE, echo=FALSE,out.width="90%", fig.cap="Landing page for the containerized Shiny Server deployment."}
...
FIXME: include reference to chapter: organizing apps/Shinylive
...
Shinylive based on a Shiny app inside the `<input-folder>` folder.
...
As you saw before, you need to have a proper wenserver hosting the pages and
...
GitLab Pages, Netlify, or the DigitalOcean App Platform. These platform all
...
The same `Dockerfile` would work all static Shiny output whether that is coming
...
`--from=<stage>` flag for the `COPY` instruction. `docker build` will then
...
tools like Docker Compose or Kubernetes, a failed healthcheck results in
creating a new container and removing the old one.

### Multi-stage Builds

A common use case for multi stage builds is to run the building/rendering
...
that one has to install. Refer to the brevious Quarto examples or
the example website for the corresponding changes.
