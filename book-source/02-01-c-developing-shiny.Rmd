...
(Fig. \@ref(fig:part2-rstudio-r)) or VS Code|index{VS COde} with the Shiny extension 
...
port number as an argument, e.g. `R -q -e "runApp(..., path = 8080)"` will
...
side can communicate with the the R process via this connection.
...
```{r part2-shiny-lifecycle, eval=TRUE, echo=FALSE,out.width="100%", fig.pos = "bt", fig.cap="The Shiny app life cycle with websocket connection."}
if (is_latex_output()) {
...
## Sharing the Shiny App Code {#part2-sharing-shiny-code}
...
However, sharing Shiny apps this ways leaves installing dependencies up to the user.
