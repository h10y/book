...
```{r part2-docker-architecture, eval=TRUE, echo=FALSE,out.width="100%", fig.pos = "bt", fig.cap="Docker architecture. Follow the solid, scattered and dotted lines to see how the Docker command line interface (CLI) interacts with the the Docker daemon and the container registry for various commands."}
if (is_latex_output()) {
...
this so-called **base* or a **parent** image. A base images is an image created 
...
this place is what we'll call as the **build context**\index{Build context, Docker}. 
...
delivery (CI/CD) /FIXME: add ref here/.
