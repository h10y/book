# Container Images {#part2-images}

::: {.noteblock data-latex=""}
This chapter dives more into what images are
for containers. Images enable quick sharing
of Shiny applications, by bundling all the
configurations and dependencies required to
run your Shiny application. 

Images are shared
through container registries, and only need to
be built once, saving time from installing dependencies and 
configuring the Shiny application each time it is
run on a new computer.
:::

## Working with Existing Images {#part2-existing-images}
...
### Image Names and Tags {#part2-docker-image-names-tags}
...
in production to always explicitly use a tag that is not `latest` but
a specific version.
...
is named as `rocker/r-base` where `rocker` is the organization namespace,
`r-base` is the repository.

Another example is the R version of the Old Faithful 
example which has the image name `ghcr.io/h10y/faithful/r-shiny:latest`.
This means:
...
<!--
REVIEW:
- There isn't enough explanation of why containers/Docker are useful. Before jumping into what Docker is, I would add a section that sets the stage that explains why containerization is such a useful concept
- This may be directly related to the previous point, but reading through this chapter really feels like I don't understand why I'm learning so much about Docker. From the perspective of a reader: I bought a book about hosting shiny, and now there are entire pages about some software that I just don't understand how it relates to hosting and why it's so important. The motivation behind Docker must be made much stronger.
- After reading the entire section about pulling an image, I didn't get a good idea of what it means to pull an image. What does that action do, why do I need to do it?
-->

### Pulling an Image {#part2-docker-pull}
...
### Docker Login {#part2-docker-login}
...
having to change login passwords elsewhere.
...
### Running a Container {#part2-docker-run}
...
## Building a New Image {#part2-docker-build}
...
of the Old Faithful app from Chapter \@ref(part2-developing-shiny-apps)
will have a `Dockerfile` and instructions in the `README.md` files within each
folder.

<!--
REVIEW:
- When building an image from a local Dockerfile for the first time, the given command is `docker build -t r-shiny:v1` but in the explanation it says that we are giving an image name of "r-shiny-test"
- The paragraph talking about the `--platform` flag and different architectures adds more confusion to the mix for the average user in my opinion. You either need to give a short introduction to what ARM vs AMD is, or preface this section by saying that this is only relevant if you already understand what these terms mean. For many readers this will be gibberish.
-->

### R for Shiny {#part2-docker-r-shiny}
...
We will explain the `Dockerfile` instructions in the next section.
...
this place is what we call the **build context**\index{Build context, Docker}. 
...
### Buildx and BuildKit {#part2-docker-buildx-buildkit}
...
As of Docker Engine 23.0 and Docker Desktop 4.19, Buildx\index{Buildx} is the default build 
client and user interface. Buildx brings extended build capabilities with 
BuildKit\index{BuildKit}. BuildKit is the server that handles the build execution, e.g.
...
See \@ref(part1-installing-docker) for enabling virtualization on Mac OS X
to enable builds for multiple platforms.

### Inspecting the Image {#part2-docker-inspect}
...
### Python for Shiny {#part2-docker-python-shiny}
...
We'll explain each line shortly. To build and check the Docker image,
...
## Managing Images {#part2-docker-managing-images}

There are a few commands that you need to know to manage your Docker images 
...
<!--
REVIEW:
- You should sample output, and you mention the "name" of images. But the outputs doesn't have a "name" column, it has a "repository" column. When the repository has a path (such as "ghcr.io/h10y/faithful/py-shiny"), it's not clear if "name" means the entire repository or just the last part of the path. I would clarify.
-->

## Sharing Images {#part2-docker-sharing-images}
...
<!--
REVIEW:
- The first sentence says that we saw that "Docker images are just compressed files linked by metadata". I re-read the previous sections a few times and I don't think this conclusion can be reached from the text.
- The section about pushing images (6.5.1) should be moved later, it should be AFTER all the sections that talk about registries, what they are, what are the options, how to access them
- When listing the different types of Docker registries, using bullet point formatting would make more sense than paragraphs.
- When creating a local registry, it's not clear what is meant by the sentence "just pull the registry image". It can be confusing, I myself didn't understand what a "registry image" means and had to look it up for it to make sense. It needs to be explained.
- For running commands "incognito", are two spaces really required to prepend to a command? I thought only one space is enough
-->

### Pushing Images {#part2-docker-push}
...
### Docker Registries {#part2-docker-registries}
...
default. **Docker Hub**\index{Docker Hub} is a service provided by Docker for finding and sharing
...
The **GitHub Container Registry**\index{GitHub Container Registry} (GHCR) is available as part of GitHub Packages
...
Every major cloud provider offers a Docker container registry
...
### Log In to a Registry {#part2-docker-registry-login}
...
### Local Registry {#part2-docker-registry-local}
...
This chapter covers the concepts of container images and registries. 
In the following chapter, we will cover how to build an image with a Shiny app.
