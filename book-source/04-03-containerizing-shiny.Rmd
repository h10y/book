# Containerizing Shiny Apps {#part2-containerizing-shiny-apps}

::: {.noteblock data-latex=""}
The previous Chapter \@ref(part2-images) covers why images
matter in a container context.

This chapter covers creating a container image for your Shiny app
by creating a Dockerfile.
:::

<!-- FIXME: add Introduction -->

## The Dockerfile {#part2-dockerfile}
...
### The Parent Image {#part2-dockerfile-parent-image}
...
### Metadata {#part2-dockerfile-metadata}
...
delivery (CI/CD) /FIXME: add ref to chapter here/. -->

### Dependencies {#part2-dockerfile-dependencies}
...
### Directories and Files {#part2-dockerfile-files-and-dirs}
...
### Switching User {#part2-dockerfile-user}
...
### Expose a Port {#part2-dockerfile-port}
...
### Variables {#part2-dockerfile-env-and-arg}
...
### Executable and Command {#part2-dockerfile-cmd}
...
Alpine uses different compilers than Ubuntu so you'll often have to build and 
...
### R Parent Images {#part2-parent-images-r}
...
### Python Parent Images {#part2-parent-images-python}
...
<!--
REVIEW:
- Not everyone knows what "system libraries" are - give examples
-->

### Manual Installation {#part2-install-system-libraries-manual}
...
error messages to see what it is that you need to install as a prerequisite.
...
<!--
REVIEW:
- When mentioning that Rocker has "littler" installed, you should mention what that is.
- I really like that you continuously focus on best practices for sensitive information (using envvars and whitespace before commands to hide tokens)
- There's some talk about pinning R packages to specific versions, but the reasoning behind why this is a good idea is completely missing (reproducibility?)
- In 6.9.2, it says that we learned how to install from a .tar.gz file. I assume this is in reference to the commands that install from local, but tar.gz files have never been explicitly mentioned.
- You do a good job of providing a complete set of methods to install R packages, but I would also add another common method many people seem to do: using a custom script (similar to the first option, but implemented differently).
- Another thing that's missing is a discussion of why you would use each one, which one is preferred, pros/cons. The only comparison is between DESCRIPTION and {deps}
-->
...
We can install the dependencies within a Docker container by doing the 
...
Here in the Dockerfile, we copy the `requirements.txt` file to the Docker image 
...
You do not need to deal with virtual environments when using Docker, because
the whole point of using Docker is to isolate environments. We tend to put
only one project in a Docker image, and we can create multiple images for the
different projects and environments and tag them appropriately.

## Classic Shiny Apps {#part2-dynamic-shiny-apps}

Shiny apps require a runtime environment on a host server that can support
the HTTP and websocket connections required for R and Python to communicate with
the client (Fig. \@ref(fig:shiny-app-websocket)). Now that we have
...
<!--
REVIEW:
- I'm not sure what is meant by "dynamic" shiny apps, as opposed to... static shiny apps? (After reading section 6.12 I do understand, but it should be clarified because "dynamic" is really just regular shiny apps)
- It would be very useful to also include a section talking about shiny server hosting a single app. That's a very common usage I see from a lot of people. It would be good to compare that to dockerizing a shiny app
-->
...
(Fig. \@ref(fig:docker-shinyserver)). Use the links to navigate the
different apps.

```{r docker-shinyserver, eval=TRUE, echo=FALSE,out.width="100%", fig.pos = "bt", fig.cap="Landing page for the containerized Shiny Server deployment."}
include_graphics("images/docker-shinyserver/docker-shinyserver.png")
```

## Containerizing Shinylive {#part2-static-shiny-apps}

We showed in Chapter \@ref(part3-static-hosting) how to host Shinylive apps using static
...
using a slimmer parent image and using a multi-stage\index{Multi-stage build} Docker build process.
...
GitLab Pages, Netlify, or the DigitalOcean App Platform (See Chapter \@ref(part3-static-hosting)). 
These platforms all offer a free tier for hosting static assets. 
...
<!--
REVIEW:
- I found this chapter quite unclear. The first few paragraphs are talking about how to host static apps on non-Docker solutions. I found this confusing because we're in a Docker chapter, so I wasn't sure if it was trying to say that these steps are done in conjunction with Docker, or as alternatives to Docker.
- This chapter introduces a new Docker concept, stages and multi-stage builds. This concept should be introduced much earlier, when teaching the concepts of Docker, rather than here
- The first paragraph does not explain what "image analysis" is. This chapter feels very out of place, at least with the lack of background provided. Perhaps this makes more sense to be placed either much earlier when teaching Docker concepts, or later as "advanced Docker concepts"?
-->
...
(Fig. \@ref(fig:docker-layers).

```{r docker-layers, eval=TRUE, echo=FALSE,out.width="100%", fig.pos = "bt", fig.cap="Docker layers for single (left) and multi-stage builds (right). Dashed lines are temporary layers."}
if (is_latex_output()) {
    include_graphics("images/docker-layers/docker-layers.pdf")
} else {
    include_graphics("images/docker-layers/docker-layers.png")
}
```
...
We have seen many examples for containerizing Shiny apps.
The next chapter will cover running these apps locally using containers.
